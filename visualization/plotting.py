...
def plot_overfitting_analysis(development_test_metrics, output_dir):
    """
    Create visualizations to analyze overfitting
    
    Parameters:
    -----------
    development_test_metrics : dict
        Dictionary with development and test metrics
    output_dir : str
        Directory to save the plots
    """
    os.makedirs(output_dir, exist_ok=True)
    ...
    model_names = list(development_test_metrics.keys())
    ...
    for model_name in model_names:
        ...
        for metric in metrics_list:
            model_diffs[metric] = development_test_metrics[model_name][metric]['diff']
    ...
    sns.heatmap(diff_df, annot=True, fmt=".4f", 
                ...
                vmin=vmin, vmax=vmax,
                linewidths=.5, 
                cbar_kws={'label': 'Development-Test Difference'})
    
    plt.title('Model Overfitting: Difference Between Development and Test Metrics', fontsize=16, pad=20)
    ...
    # Create bar charts comparing development and test metrics
    for metric in metrics_list:
        plt.figure(figsize=(16, 10))
        
        # Prepare data
        development_values = [development_test_metrics[model][metric]['development'] for model in model_names]
        test_values = [development_test_metrics[model][metric]['test'] for model in model_names]
        diff_values = [development_test_metrics[model][metric]['diff'] for model in model_names]
        ...
        # Plot bar charts
        plt.bar(x - width/2, development_values, width, label='Development Set', color='skyblue')
        plt.bar(x + width/2, test_values, width, label='Test Set', color='lightcoral')
        
        # Determine the maximum value for y-axis limit
        max_value = max(max(development_values), max(test_values))
        ...
        # Add data labels
        for j, value in enumerate(development_values):
            if value > 0.05:
        ...
        plt.title(f'{metric} - Development vs. Test Set', fontsize=14, pad=20)
        ...
        plt.savefig(f'{output_dir}/development_test_{metric}_comparison.png', 
                    dpi=300, bbox_inches='tight', pad_inches=1.0)
        plt.savefig(f'{output_dir}/development_test_{metric}_comparison.svg', 
