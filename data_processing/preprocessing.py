...
def handle_missing_values(merged_data, columns_to_exclude, random_state=42):
    ...
    continuous_vars = [col for col in X_train.columns if col not in categorical_vars + excluded_columns]
    
    # Impute missing values - KNN for continuous variables
    imputer_numeric = KNNImputer(n_neighbors=5)
    X_train_numeric = X_train[continuous_vars].copy()
    X_train_numeric_imputed = pd.DataFrame(
        imputer_numeric.fit_transform(X_train_numeric), 
    ...
    if categorical_vars:
        ...
        X_train_categorical_imputed = pd.DataFrame(
            imputer_categorical.fit_transform(X_train_categorical),
        ...
        X_train_categorical_imputed = pd.DataFrame(index=X_train.index)
        X_test_categorical_imputed = pd.DataFrame(index=X_test.index)
    
    # Get non-numeric columns
    X_train_non_numeric = X_train[excluded_columns].copy() if excluded_columns else pd.DataFrame(index=X_train.index)
    ...
    X_train_imputed = pd.concat([X_train_non_numeric, X_train_numeric_imputed, X_train_categorical_imputed], axis=1)
    X_test_imputed = pd.concat([X_test_non_numeric, X_test_numeric_imputed, X_test_categorical_imputed], axis=1)
    
    # Create complete train and test datasets with target variables
    train_data = pd.concat([X_train_imputed, pd.Series(y_train, name='death_within_28_days')], axis=1)
    ...
    train_data['survival_time'] = merged_data.loc[train_indices, 'survival_time'].values
    train_data['event'] = merged_data.loc[train_indices, 'event'].values
    ...
    return X_train_imputed, X_test_imputed, y_train, y_test, train_data, test_data