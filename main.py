...
def main(config):
    ...
    X_development, X_test, y_development, y_test, development_data, test_data = handle_missing_values(
        merged_data, columns_to_exclude, random_state=config['random_state']
    )
    
    # Save processed data
    joblib.dump(development_data, os.path.join(results_dir, 'development_data.pkl'))
    ...
    continuous_features = get_continuous_columns(X_development)
    categorical_features = get_categorical_columns(X_development)
    ...
    X_development_linear = X_development.copy()
    X_test_linear = X_test.copy()
    X_development_tree = X_development.copy()
    X_test_tree = X_test.copy()
    
    # Train models with separate datasets for different model types
    trained_models, calibrated_models = train_models(
        models, X_development_linear, X_development_tree, y_development, models_dir
    )
    ...
    development_test_metrics = analyze_overfitting(
        calibrated_models, X_development_linear, X_development_tree, 
        X_test_linear, X_test_tree, y_development, y_test, results_dir
    )
    ...
    plot_overfitting_analysis(development_test_metrics, figures_dir)
