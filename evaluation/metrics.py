...
def analyze_overfitting(models, X_train_linear, X_train_tree, X_test_linear, X_test_tree, 
                        y_train, y_test, output_dir):
    """
    Analyze overfitting by comparing model performance on training and test sets
    
    Parameters:
    -----------
    models : dict
        Dictionary of trained models
    X_train_linear, X_train_tree : DataFrame
        Training features
    ...
    for model_name, model in models.items():
        ...
        metrics['AUC'] = {'train': train_auc, 'test': test_auc, 'diff': train_auc - test_auc}
        
        # PRC
        train_prc = average_precision_score(y_train, train_pred_proba)
        test_prc = average_precision_score(y_test, test_pred_proba)
        metrics['PRC'] = {'train': train_prc, 'test': test_prc, 'diff': train_prc - test_prc}
        
        # Accuracy
        train_acc = accuracy_score(y_train, train_pred)
        test_acc = accuracy_score(y_test, test_pred)
        metrics['Accuracy'] = {'train': train_acc, 'test': test_acc, 'diff': train_acc - test_acc}
        
        # Precision (PPV)
        train_prec = precision_score(y_train, train_pred)
        test_prec = precision_score(y_test, test_pred)
        metrics['Precision'] = {'train': train_prec, 'test': test_prec, 'diff': train_prec - test_prec}
        
        # Recall (Sensitivity)
        train_recall = recall_score(y_train, train_pred)
        test_recall = recall_score(y_test, test_pred)
        metrics['Recall'] = {'train': train_recall, 'test': test_recall, 'diff': train_recall - test_recall}
        
       # F1 Score
        train_f1 = f1_score(y_train, train_pred)
        test_f1 = f1_score(y_test, test_pred)
        metrics['F1 Score'] = {'train': train_f1, 'test': test_f1, 'diff': train_f1 - test_f1}
        
        # Store results
        train_test_metrics[model_name] = metrics
        
        # Print results
        print(f"Model: {model_name}")
        print(f"AUC - Train: {train_auc:.4f}, Test: {test_auc:.4f}, Difference: {train_auc - test_auc:.4f}")
        print(f"PRC - Train: {train_prc:.4f}, Test: {test_prc:.4f}, Difference: {train_prc - test_prc:.4f}")
        print(f"Accuracy - Train: {train_acc:.4f}, Test: {test_acc:.4f}, Difference: {train_acc - test_acc:.4f}")
        print(f"Precision - Train: {train_prec:.4f}, Test: {test_prec:.4f}, Difference: {train_prec - test_prec:.4f}")
        print(f"Recall - Train: {train_recall:.4f}, Test: {test_recall:.4f}, Difference: {train_recall - test_recall:.4f}")
        print(f"F1 Score - Train: {train_f1:.4f}, Test: {test_f1:.4f}, Difference: {train_f1 - test_f1:.4f}")
    
    # Save overfitting analysis results
    joblib.dump(train_test_metrics, f'{output_dir}/overfitting_analysis.pkl')
